...
- [AT91SAM7S256D-AU](https://www.microchip.com/en-us/product/AT91SAM7S256){:target="_blank"} - the MCU. ARM7TDMI, 256 KB Flash, 64 KB SRAM, 64-LQFP, on an 18.432 MHz crystal (exactly the value the PLL needs to synthesize the 48 MHz USB clock). Runs the whole show and presents the USB device port to the radio. Its logic section runs from USB VBUS at under 100 mA.
- [ADuM1410](https://www.analog.com/en/products/adum1410.html){:target="_blank"} - Analog Devices iCoupler, a 4-channel digital isolator. This is the barrier between the radio-referenced logic and the vehicle-referenced coil drive. Relay commands cross it outbound; relay coil supply power-good status comes back on a dedicated channel.
- [TPS3808G01](https://www.ti.com/product/TPS3808){:target="_blank"} - adjustable voltage supervisor in SOT-23-6. One sits on the isolated side watching the 9 V coil rail and returns a power-good signal to the MCU over an ADuM1410, in addition to a couple located on the USB side.
- [DS2433](https://www.analog.com/en/products/ds2433.html){:target="_blank"} - 4 Kb 1-Wire EEPROM. Holds the GCAI accessory ID the radio reads to decide whether to drive the port as USB or RS-232.
...
My replacement keeps the same two-domain shape as Motorola's, with isolated logic on one side, coil drive on the other. The full [schematic is on github](https://github.com/aramder/orc/blob/main/hardware/orc.pdf){:target="_blank"}.
...
- **[SN65HVD230](https://www.ti.com/product/SN65HVD230){:target="_blank"}** - 3.3 V CAN transceiver. Bus ESD protection is built into the part, and a 120 ohm termination resistor can be connected via a removable jumper.
- **[ADuM1250](https://www.analog.com/en/products/adum1250.html){:target="_blank"}** - hot-swappable isolated I2C buffer. This is my whole isolation barrier: instead of running ten command lines through a bank of isolator channels the way Motorola did, only SDA and SCL cross, and everything that drives the channels lives on the coil side and gets addressed over that one bus.
- **[PCA9555](https://www.nxp.com/products/PCA9555){:target="_blank"}** - 16-bit I2C GPIO expander at address 0x20, on the isolated side. Ten of its sixteen pins are the channel enables. Reading its input port back is also how the firmware reports applied state, so status comes from the hardware rather than from a variable.
- **[AO3401A](https://www.aosmd.com/sites/default/files/res/datasheets/AO3401A.pdf){:target="_blank"} x10** - the high-side switches. The relay coil returns are common on the relay board, so switching has to happen between +9 V and the top of each coil, which rules out the usual low-side sink array. Each P-FET is turned on by an MMBT3904 level shifter from an expander pin.
- **[SS34](https://www.onsemi.com/download/data-sheet/pdf/ss39-d.pdf){:target="_blank"} x10** - flyback diode. Not ideal for this to be all the way on another board, but whatever.
- **[LMR50410](https://www.ti.com/product/LMR50410){:target="_blank"} x2** - a SOT-23-6 synchronous buck used twice: once for 3.3 V logic, once for the 9 V coil rail off the harness A+ tap. Both are designed around a 28.8 V input ceiling, so the board works on a 24 V system as well as a 12 V one.
...
The source for this project, both the firmware and hardware, is available on github at [aramder/orc](https://github.com/aramder/orc){:target="_blank"}.
