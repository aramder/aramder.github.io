...
The side effect was unexpected: FPS jumped from ~12 to ~17 (+42%). A larger heap means fewer texture cache evictions, so `R_GenerateComposite` — which rebuilds multi-patch wall textures from the WAD — gets called far less often. A memory fix produced a performance fix.
